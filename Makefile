...
$(BIN)/%: | $(BIN) ; $(info $(M) building $(REPOSITORY)…)
	$Q tmp=$$(mktemp -d); \
		(env GO111MODULE=off GOPATH=$$tmp $(GO) get $(REPOSITORY) \
			&& cp $$tmp/bin/* $(BIN)/.) || ret=$$?; \
		rm -rf $$tmp ; exit $$ret
...
GODEPGRAPH = $(BIN)/godepgraph
$(BIN)/godepgraph: REPOSITORY=github.com/kisielk/godepgraph
...
.PHONY: depgraph
depgraph: | $(GODEPGRAPH); $(info $(M) running godepgraph…) @ ## Generate dependencies graph
	$(GODEPGRAPH) -s -o github.com/catsoap/bsaig github.com/catsoap/bsaig | dot -Tpng -o depgraph.png
